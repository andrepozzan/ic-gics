...
def calcMethodByPandM(P, M):
    ...
    return np.max(np.abs(vector_error))
...
for p in range(1, P_MAX+1):  # P vai de 1 a 5, incrementando de 1 em 1
    for m in range(1, M_MAX+1):  # M vai de 1 a 3, incrementando de 1 em 1
        print(f"p: {p}, m: {m}")
        max_error = calcMethodByPandM(p, m)
        errorsResult.append((p, m, max_error))

        # Gerar gráfico para a configuração atual
        ax.scatter([p], [m], [max_error], marker='o', s=45)

ax.set_xlabel('P')
ax.set_ylabel('M')
ax.set_zlabel('Max Error')
ax.set_title('3D Plot of P, M, and Max Error')

# Ajustar o grid para que P e M sejam inteiros
