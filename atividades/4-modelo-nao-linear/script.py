        ...
        for p in range(1, P + 1):  # Incluir termos até a ordem P
            for m in range(M + 1):  # Considerar atrasos até M
                ...
                if dataIndex >= 0:  # Verificar se o índice é válido
                    line_of_regression_matrix.append(
                        in_data[dataIndex, 0] ** p)
                else:
                    # Adicionar zero se o índice for inválido
                    line_of_regression_matrix.append(0)
    ...
    for n in range(len(x)):
        ...
        for p in range(1, P + 1):  # Incluir termos até a ordem P
            for m in range(M + 1):  # Considerar atrasos até M
                ...
                if dataIndex >= 0:  # Verificar se o índice é válido
                    # Equação de Volterra
                    estimated_value += coef_matrix[p - 1, m] * (
                        x[dataIndex] ** p)
...
def calcCoefByLeastSquares():
    result = least_squares(mp_residuals, initial_estimated, args=(
        in_training.ravel(), out_training.ravel(), P, M), verbose=2)
...
def checkError(vector_error):
    ...
    max_error_point_original = (
        in_training[max_error_index], out_training[max_error_index])
    max_error_point_estimated = (
        in_training[max_error_index], out_estimated_volterra[max_error_index])
    min_error_point_original = (
        in_training[min_error_index], out_training[min_error_index])
    min_error_point_estimated = (
        in_training[min_error_index], out_estimated_volterra[min_error_index])
...
def plotErrorPair(p1, p2, color, label_base, marker1="x", marker2="o", zorder=5):
    ...
    plt.plot([x1, x2], [y1, y2], color=color, linestyle='--',
             linewidth=1.5, label=f'Linha de {label_base}', zorder=zorder-1)


coef = calcCoefByLeastSquares()
regression_matrix = createRegressionMatrix(
    in_training, P, M)
out_estimated_volterra = np.dot(regression_matrix, coef)

vector_error = calcVectorError(out_estimated_volterra)
max_error_point_original, max_error_point_estimated, min_error_point_original, min_error_point_estimated = checkError(
    vector_error)

plotErrorPair(max_error_point_original, max_error_point_estimated,
              color='green', label_base='Erro Máximo')
plotErrorPair(min_error_point_original, min_error_point_estimated,
              color='red', label_base='Erro Mínimo')

plt.scatter(in_training, out_training,
            label='Dados Originais', color='blue', s=100)
plt.scatter(in_training, out_estimated_volterra,
            color='orange', label='Ajuste', alpha=0.8, s=100)
plt.xlabel('in_training', fontsize=30)
plt.ylabel('out_training', fontsize=30)
plt.title(
    'Dados Originais e Estimados com Erros', fontsize=36)
