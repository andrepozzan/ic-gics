...
plt.scatter(np.abs(in_validation), np.abs(out_validation), label="Measured data", s=100)
plt.scatter(np.abs(in_validation), np.abs(saidaEstimadaPolinomio), label="Estimated", alpha=0.7, s=100)
plt.xlabel("Input Amplitude", fontsize=30)
plt.ylabel("Output Amplitude", fontsize=30)
plt.title("AM-AM Graph", fontsize=36)
...
plt.scatter(np.abs(in_validation), delta_fase, label="Phase difference", s=100)
plt.scatter(np.abs(in_validation), delta_fase_estimada, label="Estimated", alpha=0.7, s=100)
plt.xlabel("Input Amplitude", fontsize=30)
plt.ylabel("Phase difference", fontsize=30)
plt.title("AM-PM Graph", fontsize=36)
plt.legend(fontsize=30, markerscale=2)
plt.grid()
plt.tick_params(axis='both', which='major', labelsize=25)
plt.show()
